<x-app-layout>
    ...
    <section class="hero-section text-white min-h-screen flex items-center relative overflow-hidden pt-16">
        ...
        <div class="absolute inset-0">
            <img src="{{ asset('images/bG_HERO.webp') }}" alt="Hero Background" class="w-full h-full object-cover object-top">
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                <div class="text-center lg:text-left animate-fade-in-up">
                    ...
                    <div class="flex items-center justify-center lg:justify-start gap-2 mb-2">
                        <img src="{{ asset('images/LOGO_SIGAP.png') }}" alt="SIGAP Logo" class="w-12 h-12 md:w-16 md:h-16 object-contain drop-shadow-2xl">
                        ...
                        <div class="relative z-10">
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                <div class="bg-white/10 rounded-xl p-6 text-center border border-white/20 hover:bg-white/20 transition-all transform hover:scale-105">
                                    <div class="text-5xl font-extrabold text-sigap-orange mb-2">{{ \App\Models\Job::where('status', 'open')->count() }}+</div>
                                    <div class="text-sm text-blue-100 font-medium">Pekerjaan Aktif</div>
                                </div>
                                <div class="bg-white/10 rounded-xl p-6 text-center border border-white/20 hover:bg-white/20 transition-all transform hover:scale-105">
                                    <div class="text-5xl font-extrabold text-white mb-2">{{ \App\Models\User::where('role', 'pekerja')->count() }}+</div>
    ...
    <section id="pekerjaan" class="py-20 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        @foreach($urgentJobs as $job)
                            <div class="bg-gradient-to-br from-red-50 to-orange-50 border-2 border-red-200 rounded-2xl p-6 hover:border-red-400 hover:shadow-2xl transition-all duration-300 fade-on-scroll transform hover:-translate-y-2 relative">
                                <div class="flex items-start justify-between mb-4">
                                    <span class="text-xs font-bold px-4 py-1.5 bg-sigap-blue/10 text-sigap-blue rounded-full">{{ $job->category }}</span>
                                    <span class="text-xs text-gray-500 font-medium">{{ $job->created_at->diffForHumans() }}</span>
                                </div>
                                <h3 class="text-xl font-bold text-sigap-blue mb-3 line-clamp-2 min-h-[3.5rem]">
                                    {{ $job->title }}
                                </h3>
                                <p class="text-sm text-gray-600 mb-5 line-clamp-2 min-h-[2.5rem]">{{ Str::limit($job->description, 100) }}</p>
                                ...
                                @auth
                                    @if(auth()->user()->isPekerja())
                                        <a href="{{ route('jobs.show', $job) }}" class="block w-full text-center bg-red-500 hover:bg-red-600 text-white font-bold py-3 px-4 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105">
                                            Lamar Sekarang
                                        </a>
                                    @else
                                        <a href="{{ route('jobs.show', $job) }}" class="block w-full text-center bg-sigap-blue hover:bg-blue-900 text-white font-bold py-3 px-4 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105">
                                            Lihat Detail
                                        </a>
                                    @endif
                                @else
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        @foreach($latestJobs as $job)
                            <div class="bg-white border-2 border-gray-200 rounded-2xl p-6 hover:border-sigap-orange hover:shadow-2xl transition-all duration-300 fade-on-scroll transform hover:-translate-y-2">
                                ...
                                @auth
                                    @if(auth()->user()->isPekerja())
                                        <a href="{{ route('jobs.show', $job) }}" class="block w-full text-center bg-sigap-orange hover:bg-orange-600 text-white font-bold py-3 px-4 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105">
                                            Lamar Sekarang
                                        </a>
                                    @else
                                        <a href="{{ route('jobs.show', $job) }}" class="block w-full text-center bg-sigap-blue hover:bg-blue-900 text-white font-bold py-3 px-4 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105">
            ...
            <div class="text-center">
                <a href="{{ route('jobs.index') }}" class="inline-block bg-sigap-blue hover:bg-blue-900 text-white font-bold py-4 px-10 rounded-xl transition-all duration-300 shadow-xl hover:shadow-2xl transform hover:-translate-y-1 text-lg">
                        ...
                        <div class="testimonial-item flex-shrink-0 w-full md:w-1/2 lg:w-1/3">
                            <div class="bg-white rounded-2xl p-8 shadow-xl border border-gray-100 h-full">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <p class="font-bold text-sigap-blue">{{ $testimonial['name'] }}</p>
                    ...
                    @foreach($partners as $partner)
                        <div class="partner-item flex-shrink-0">
                            <div class="bg-gray-50 rounded-xl p-6 w-48 h-32 flex items-center justify-center border-2 border-gray-200 hover:border-sigap-orange transition-all">
                                <span class="text-xl font-bold text-sigap-blue">{{ $partner }}</span>
                            </div>
                        </div>
                    @endforeach
