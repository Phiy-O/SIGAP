<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-8">
        <div class="mb-6">
            @auth
                @if(auth()->user()->id === $job->user_id)
                    <a href="{{ route('dashboard') }}" class="text-sigap-orange hover:underline flex items-center">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                        </svg>
                        Kembali ke Dashboard
                    </a>
                @elseif(auth()->user()->isPekerja())
                    <a href="{{ route('dashboard') }}" class="text-sigap-orange hover:underline flex items-center">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                        </svg>
                ...
                @else
                    <a href="{{ route('jobs.index') }}" class="text-sigap-orange hover:underline flex items-center">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                        </svg>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        @auth
                            @if(auth()->user()->id === $job->user_id)
                                <div class="flex gap-2">
                                    <a href="{{ route('jobs.edit', $job) }}" class="btn-outline text-sm">
                                        Edit
                                    </a>
            ...
            <div class="lg:col-span-1">
                <div class="card sticky top-20">
                    ...
                    @auth
                        @if(auth()->user()->isPekerja() && $job->status == 'open')
                            @if($hasApplied)
                                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                                    <p class="text-sm text-yellow-800">Anda sudah melamar pekerjaan ini</p>
                                </div>
                            @else
                                <form method="POST" action="{{ route('jobs.apply', $job) }}" class="mb-4">
                                    @csrf
                                    <div class="mb-4">
                            ...
                            @endif
                        @elseif(auth()->user()->isPekerja() && $job->status == 'in_progress' && $myApplication && $myApplication->status == 'accepted')
                            @if($myApplication->proof_photo)
                                <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                                    ...
                                    <a href="{{ asset('storage/' . $myApplication->proof_photo) }}" target="_blank" class="text-sm text-green-700 hover:text-green-900 underline">
                                        Lihat Bukti Foto
                                    </a>
                                </div>
                            @else
                        ...
                        @elseif(auth()->user()->id === $job->user_id)
                            <div class="space-y-2 mb-4">
                                <p class="text-sm text-gray-600 mb-2">Total Lamaran: <strong>{{ $job->applications->count() }}</strong></p>
                                @if($job->applications->where('status', 'pending')->count() > 0)
                                    <p class="text-sm text-yellow-600 font-medium">
                                        {{ $job->applications->where('status', 'pending')->count() }} lamaran pending
                                    </p>
                                @endif
                                @if($job->applications->where('status', 'accepted')->count() > 0)
                                    <p class="text-sm text-green-600 font-medium">
                                        {{ $job->applications->where('status', 'accepted')->count() }} lamaran diterima
                                    </p>
                                @endif
                            </div>
                    ...
                    @else
                        @if($job->status == 'open')
                            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                ...
                                <div class="flex gap-2">
                                    <a href="{{ route('login') }}" class="btn-primary text-sm py-2 px-4 flex-1 text-center">
                                        Login
                                    </a>
                                    <a href="{{ route('register') }}" class="btn-secondary text-sm py-2 px-4 flex-1 text-center">
                                        Daftar
                                    </a>
                                </div>
                            </div>
        ...
        @auth
            @if(auth()->user()->id === $job->user_id && $job->applications->count() > 0)
                                    ...
                                    <div class="flex justify-between items-start mb-4">
                                        ...
                                        <div class="ml-4 text-right">
                                            ...
                                            @if($application->status == 'pending' && $job->status == 'open')
                                                <form method="POST" action="{{ route('applications.accept', $application) }}" class="mt-2">
                                                    @csrf
                                                    <button type="submit" 
        ...
        <!-- Complete Job Modal -->
        @auth
            @if(auth()->user()->isPekerja() && $job->status == 'in_progress' && $myApplication && $myApplication->status == 'accepted' && !$myApplication->proof_photo)
                <div id="complete-modal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
                    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                        <div class="mt-3">
                            ...
                            <form method="POST" action="{{ route('jobs.complete', $job) }}" enctype="multipart/form-data" id="complete-form">
                                @csrf
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="file" id="proof_photo" name="proof_photo" accept="image/*" required
                                        class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-sigap-orange file:text-white hover:file:bg-orange-600 @error('proof_photo') border-red-500 @enderror">
                    ...
                    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                        <div class="mt-3">
                            ...
                            <p class="text-sm text-gray-600 text-center mb-6">
                                Apakah Anda yakin ingin menghapus pekerjaan "<strong>{{ $job->title }}</strong>"? 
                                Tindakan ini tidak dapat dibatalkan dan semua data terkait akan dihapus.
                            </p>
                            
                            <form method="POST" action="{{ route('jobs.destroy', $job) }}" class="flex gap-3">
                                @csrf
                                @method('DELETE')
