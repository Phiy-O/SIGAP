            ...
            <div class="card">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-sigap-blue mt-2">{{ $myJobs->count() }}</p>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-sigap-orange mt-2">
                            {{ $myJobs->where('status', 'open')->count() }}
                        </p>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-yellow-600 mt-2">
                            {{ $pendingApplications->sum(fn($job) => $job->applications->where('status', 'pending')->count()) }}
                        </p>
                    </div>
                ...
                @if(auth()->user()->canSwitchRole())
                    <form method="POST" action="{{ route('role.switch') }}" class="inline">
                        ...
                        <div class="flex items-center gap-2">
                            <span class="text-sm text-gray-600">Role saat ini: <strong>{{ ucfirst(auth()->user()->role) }}</strong></span>
                            <button type="submit" name="role" value="{{ auth()->user()->isUser() ? 'pekerja' : 'user' }}" 
                                class="text-sm bg-sigap-orange hover:bg-orange-600 text-white px-4 py-2 rounded-lg transition">
                                Switch ke {{ auth()->user()->isUser() ? 'Pekerja' : 'User' }}
                            </button>
                        </div>
                    </form>
                @endif
            </div>
            <a href="{{ route('jobs.create') }}" class="btn-primary inline-flex items-center">
        ...
        <div class="card">
                        ...
                        <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                            <div class="flex justify-between items-start">
                                <div class="flex-1">
                                    ...
                                    <p class="text-gray-600 text-sm mb-2">{{ Str::limit($job->description, 100) }}</p>
                                ...
                                <div class="ml-4 text-right">
                                    ...
                                    @endif
                                    <div class="mt-2">
                                        <a href="{{ route('jobs.show', $job) }}" class="text-sm text-sigap-orange hover:underline">
                                            Lihat Detail
                                        </a>
                                    </div>
                                </div>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <a href="{{ route('jobs.create') }}" class="btn-primary inline-block">
